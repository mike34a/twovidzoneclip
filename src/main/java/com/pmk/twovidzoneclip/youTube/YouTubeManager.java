...
public class YouTubeManager {
    ...
    public YouTubeManager(String clientID) {
     ...
     public List<YouTubeVideo> retrieveVideos(String textQuery, int maxResults, boolean filter, int timeout) throws Exception {
        ...
        return convertVideos(videos);
          ...
          public List<VideoEntry> retrieveVideos2(String textQuery, int maxResults, boolean filter, int timeout) throws Exception {
    ...
    public boolean checkUrl(String url) throws Exception {
        List<VideoEntry> video = this.retrieveVideos2(url, 1, true, 2000);
        if (video.isEmpty()) return false;
        if(!video.get(0).isEmbeddable()) return false;
        return true;
    }
